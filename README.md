...
This setup uses Cloudflare as the hosting service, and [DNSimple] as the
...
1. **Get a domain on [DNSimple].** Follow the instructions on the website to
   create an account and buy a domain, then navigate to your account page and
   create an access token. Add that token to `.envrc.private` as
   ...
   to match, as well as adapt the following steps.
2. **Create a Cloudflare account.** Set the `CLOUDFLARE_API_KEY` env var to
   your Cloudflare API Key, and the `CLOUDFLARE_EMAIL` env var to your email
   (the one you gave to Cloudflare).
...
3. **Create a bucket to store the Terraform state.** In Cloudflare, create a
   bucket under "R2 Object Storage" (default settings are fine for this
   bucket). Update `tf/main.tf` to set the name of the bucket (`bucket` under
   `backend`) and the folder (`key` under `backend`) within the bucket that
   will be used to store your Terraform state.
4. **Tell Terraform about it.** On the R2 Object Storage "Overview" page, at
   the bottom, click the "manage" button next to "API Tokens", and create a
   token for the new bucket. The token needs "Object Read & Write" permissions
   on the specific bucket you just created. Create env vars `AWS_ACCESS_KEY_ID`
   with the value displayed as "Access Key ID", `AWS_SECRET_ACCESS_KEY` with
   the value for "Secret Access Key", and "TF_VAR_s3_endpoint" with the value
   for "jurisdiction-specific endpoint". These are only shown once; if you
   missed them, simply create a new token (and delete the old one).
6. **Explicitly tell Cloudflare about your Account ID.** Add your Cloudflare
   Account ID as both `CLOUDFLARE_ACCOUNT_ID` and
   ...
   your account name.
7. **Check you replaced all the `__CHANGE_ME__`s in `main.tf`.** Those were: in
   the `backend` block, `bucket` and `key`; in the `locals` block, `domain`.
8. **Set up DNS.** In the `tf` folder, run `tofu init`, then `tofu apply`. If
...
custom domain name. If you do not want a custom domain, remove the `pattern`
map entirely (resulting in an empty array for `routes`).

Double-check there is no `__CHANGE_ME__` value left, then run

```
deploy
```

anywhere in this repo, which will run the `bin/deploy` script.
